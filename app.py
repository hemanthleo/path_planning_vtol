...
def generate_simplified_path(
    ...
    if is_unusual_terrain(elevs):
        ...
        safe_alt_rel = (max_elev - home_elev) + 100
    current_step += 1
    if progress_callback:
    ...
    path.append((pre_entry_loiter[0], pre_entry_loiter[1], safe_alt_rel))
    trigger_points.append(
        {
    ...
    num_samples = max(20, int(seg_length / 5))
    ...
    if not valid_first_seg_elevs:
        ...
        cruise_rel_first = 100 + (safety_margin if safety_margin is not None and safety_margin > 0 else FALLBACK_SAFETY_MARGIN)
    else:
    ...
    current_step += 1
    if progress_callback:
        progress_callback(current_step / total_steps, "Set cruise altitude for first segment")
    ...
    for seg_idx, seg in enumerate(lines):
        ...
        if not valid_seg_elevs:
            ...
            cruise_rel = 100 + (safety_margin if safety_margin is not None and safety_margin > 0 else FALLBACK_SAFETY_MARGIN)
        else:
    ...
    if is_unusual_terrain(elevs):
        ...
        max_elev_coords = sample_coords[int(np.argmax([e if e is not None and -500 < e < 9000 else -float('inf') for e in elevs]))]
        new_alt_rel = (max_elev - home_elev) + 100
    path.append((exit_pt[0], exit_pt[1], new_alt_rel))
    trigger_points.append(
        {
            "lat": exit_pt[0],
            "lon": exit_pt[1],
