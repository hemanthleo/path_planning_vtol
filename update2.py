...
ELEVATION_SAMPLING_DISTANCE = 5.0  # Meters between elevation samples
...
def initialize_session_state():
    defaults = {
        ...
        'elevation_sampling_distance': ELEVATION_SAMPLING_DISTANCE
    }
...
def generate_simplified_path(
    ...
    num_samples = max(40, int(total_dist / st.session_state.elevation_sampling_distance))
    ...
    num_samples = max(10, int(seg_length / st.session_state.elevation_sampling_distance))
    ...
    for seg_idx, seg in enumerate(lines):
        ...
        num_samples = max(30, int(seg_length / st.session_state.elevation_sampling_distance))
        a, b = seg[0], seg[-1]
        brg = calculate_bearing(a, b)
        sample_coords = [move_point_along_bearing(a, brg, seg_length * (i / num_samples)) for i in range(num_samples + 1)]
    ...
    num_samples = max(10, int(direct_dist / st.session_state.elevation_sampling_distance))
    sample_coords = [move_point_along_bearing(exit_pt, brg_to_home, direct_dist * (i / num_samples)) for i in range(num_samples + 1)]
    elevs = fetch_elevations(sample_coords)
    valid_elevs = [e for e in elevs if e is not None and -500 < e < 9000]
    if is_unusual_terrain(elevs):
...
with st.sidebar:
    ...
    elevation_sampling_distance = st.number_input(
        "Elevation Sampling Distance (m)", min_value=1.0, max_value=50.0, value=ELEVATION_SAMPLING_DISTANCE, step=0.5,
        help="Distance between elevation samples for terrain analysis (smaller values increase accuracy but require more API calls)."
    )
    st.session_state.elevation_sampling_distance = elevation_sampling_distance
